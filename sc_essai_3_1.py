...
class KamalondoVisualizations:
    ...
    def plot_voltage_comparison_realistic(self):
        """Tensions pv seul et pv + ess"""

        depart_names = ['Départ 1', 'Départ 2', 'Départ 3', 
                        'Départ 4', 'Départ 5', 'Moyenne Réseau']
        ...
        for i, depart in enumerate(['B2', 'B3', 'B4', 'B5', 'B6']):
            ...
            ax.bar(x - width/2, v_before, width, label='PV seul', color="#EE3D2A", alpha=0.8)
            ax.bar(x + width/2, v_after, width, label='PV + ESS', color="#0ED862", alpha=0.8)

            ax.axhline(y=0.95, color='black', linestyle='--', alpha=0.7, linewidth=2)
            ax.axhline(y=1.05, color='black', linestyle='--', alpha=0.7, linewidth=2)

            #ax.set_title(depart_names[i], fontweight='bold', fontsize=12)
            ax.set_xticks(x)
            ax.set_xticklabels(['phase_A', 'phase_B', 'phase_C'])
            ax.set_ylabel('Tension (pu)')
            ax.grid(True, alpha=0.3)
            ...
            for j, (v_b, v_a) in enumerate(zip(v_before, v_after)):
                ...
                if abs(improvement) > 0.001:
                    ...
                    ax.text(j, max(v_b, v_a) + 0.02, 
                            f'{improvement:+.3f}', ha='center', va='bottom',
                            fontweight='bold', color=color, fontsize=9)

            plt.tight_layout()
            plt.savefig(f'outputs_realistic/2_voltage_comparison_realistic_{depart}.png', dpi=300, bbox_inches='tight')
            plt.show()

        # Figure pour la moyenne réseau
        fig, ax = plt.subplots(figsize=(6, 4))
        v_mean_before = [np.mean([self.v_real_before[b][i] for b in ['B2', 'B3', 'B4', 'B5', 'B6']]) for i in range(3)]
        v_mean_after = [np.mean([self.v_real_after[b][i] for b in ['B2', 'B3', 'B4', 'B5', 'B6']]) for i in range(3)]

        ax.bar(x - width/2, v_mean_before, width, label='PV seul', color="#EE3D2A", alpha=0.8)
        ax.bar(x + width/2, v_mean_after, width, label='PV + ESS', color="#0ED862", alpha=0.8)
        ...
        for j, (v_b, v_a) in enumerate(zip(v_mean_before, v_mean_after)):
            improvement = v_a - v_b
            if abs(improvement) > 0.001:
                color = 'green' if improvement > 0 else 'red'
                ax.text(j, max(v_b, v_a) + 0.02, 
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/2_voltage_comparison_realistic_moyenne.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_losses_comparison_realistic(self):
        ...
        ax1.set_ylim(0, max(max(self.losses_real_before), max(self.losses_real_after)) * 1.2)
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/3a_losses_by_depart_realistic.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/3b_total_losses_realistic.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_vuf_comparison_realistic(self):
        ...
        # Figure 1 : Comparaison avant/après par départ
        ...
        ax1.legend()
        ax1.grid(True, alpha=0.3, axis='y')
        ax1.set_ylim(0, max(max(self.vuf_real_before), max(self.vuf_real_after)) * 1.2)
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/4a_vuf_by_depart_realistic.png', dpi=300, bbox_inches='tight')
        plt.show()

        # Figure 2 : Évolution temporelle du VUF critique
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/4b_vuf_temporal_realistic.png', dpi=300, bbox_inches='tight')
        plt.show()

    # 5. COURANTS RÉELS MESURÉS DANS LE RÉSEAU
    def plot_currents_realistic(self):
        """Courants mesurés dans le réseau de Kamalondo - Figures séparées"""

        departs = ['B2', 'B3', 'B4', 'B5', 'B6']
        phases = ['Phase A', 'Phase B', 'Phase C']
        colors = ['#1f77b4', '#ff7f0e', '#2ca02c']

        # Figures par départ pour les courants de phase
        for i, depart in enumerate(departs):
            fig, ax = plt.subplots(figsize=(6, 5))
            currents = self.currents_real[depart]

            bars = ax.bar(phases, currents, color=colors, alpha=0.8, edgecolor='black')
            ...
            plt.tight_layout()
            plt.savefig(f'outputs_realistic/5a_currents_phase_{depart}.png', dpi=300, bbox_inches='tight')
            plt.show()

        # Figure pour le courant neutre
        fig, ax = plt.subplots(figsize=(6, 5))
        neutral_currents = [274.4, 109.8, 13.8, 100.8, 122.9]  # Données réelles

        ax.bar(departs, neutral_currents, color='purple', alpha=0.8, edgecolor='black')
        ax.set_ylabel('Courant neutre (A)')
        ax.set_xlabel('Départs')
        ax.grid(True, alpha=0.3, axis='y')
        ax.set_ylim(0, max(neutral_currents) * 1.25)

        for i, current in enumerate(neutral_currents):
            ax.text(i, current + 10, f'{current:.1f}A', ha='center', va='bottom', fontweight='bold')

        plt.tight_layout()
        plt.savefig('outputs_realistic/5b_currents_neutral.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_losses_heatmap_realistic(self):
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/11a_losses_heatmap_avant.png', dpi=300, bbox_inches='tight')
        plt.show()

        # Figure 2 : Heatmap après optimisation
        fig2, ax2 = plt.subplots(figsize=(8, 6))
        im2 = ax2.imshow(pertes_apres, cmap='Greens', aspect='auto')
        ax2.set_xlabel('Heure', fontsize=11, fontweight='bold')
        ax2.set_ylabel('Depart', fontsize=11, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/11b_losses_heatmap_apres.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_vuf_by_depart_realistic(self):
        ...
        # Figure 1 : VUF avant/après optimisation
        fig1, ax1 = plt.subplots(figsize=(8, 6))
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/16a_vuf_comparaison_par_depart.png', dpi=300, bbox_inches='tight')
        plt.show()

        # Figure 2 : Causes du déséquilibre
        fig2, ax2 = plt.subplots(figsize=(8, 6))
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/16b_causes_du_desequilibre.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    # 19. CAPACITÉ PV PAR DÉPART - STACKED + RANKING
    def plot_pv_hosting_by_depart_realistic(self):
        """Capacité d'accueil PV par départ"""
        ...
        # Figure 1 : Capacité par phase (stacked)
        fig1, ax1 = plt.subplots(figsize=(10, 6))
        depart_labels = [f"{feeder}" for feeder in df['feeder']]
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/19a_pv_hosting_by_phase.png', dpi=300, bbox_inches='tight')
        plt.show()

        # Figure 2 : Classement par capacité totale
        fig2, ax2 = plt.subplots(figsize=(10, 6))
        df_sorted = df.sort_values('pv_total_kw', ascending=False)
        sorted_labels = [f"{feeder}" for feeder in df_sorted['feeder']]
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/19b_pv_hosting_ranking.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_pv_hosting_by_phase_depart_realistic(self):
        ...
        # Figure 1 : Barres groupées par phase
        fig1, ax1 = plt.subplots(figsize=(10, 6))
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/20a_pv_hosting_grouped_by_phase.png', dpi=300, bbox_inches='tight')
        plt.show()

        # Figure 2 : Heatmap capacité PV
        fig2, ax2 = plt.subplots(figsize=(10, 6))
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/20b_pv_hosting_heatmap.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_synthesis_results_realistic(self):
        ...
        # Figure 1 : Comparaison des résultats
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/21a_synthesis_comparison.png', dpi=300, bbox_inches='tight')
        plt.show()

        # Figure 2 : Améliorations par catégorie
        ...
        plt.tight_layout()
        plt.savefig('outputs_realistic/21b_synthesis_improvements.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    # 26. Pertes actives
    def plot_losses_comparison(self):
        fig, ax = plt.subplots(figsize=(12, 6))

        hours = list(range(24))

        losses_before = [15 + 10*np.exp(-0.5*((h-14)/3)**2) for h in hours]
        
        ax.fill_between(hours, losses_before, alpha=0.3, color='red', label='Pertes PV seul')
        ax.plot(hours, losses_before, color='red', linewidth=2, marker='o')
        
        # Modifie losses_after pour obtenir une réduction moyenne de 30%
        reduction_target = 0.3
        mean_losses_before = np.mean(losses_before)
        ...
        modified_losses_after = [loss * reduction_factor for loss in losses_before]
        
        ax.fill_between(hours, modified_losses_after, alpha=0.3, color='green', label='Pertes PV + ESS')
        ax.plot(hours, modified_losses_after, color='green', linewidth=2, marker='s')
        ...
        reduction = (np.mean(losses_before) - np.mean(modified_losses_after)) / np.mean(losses_before) * 100
        ax.text(12, max(losses_before)*0.8, f'Réduction moyenne: {reduction:.1f}%', 
                fontsize=12, ha='center', bbox=dict(boxstyle="round,pad=0.3", facecolor="white"))
        
        plt.tight_layout()
        plt.savefig('outputs_realistic/26_losses_comparison.png', dpi=300, bbox_inches='tight')
        plt.show()

...
def build_opf_model(mode: str='capacity', pv_cap_fixed: float|None=None,
    ...
    slack = min(buses)
    def slack_fix(m,p,tt):
        if p=='a': return (m.Ure[slack,p,tt]==1.0) & (m.Uim[slack,p,tt]==0.0)
        if p=='b': return (m.Ure[slack,p,tt]==-0.5) & (m.Uim[slack,p,tt]==-math.sqrt(3)/2)
        if p=='c': return (m.Ure[slack,p,tt]==-0.5) & (m.Uim[slack,p,tt]==+math.sqrt(3)/2)
        return (m.Ure[slack,p,tt]==0.0) & (m.Uim[slack,p,tt]==0.0)
    m.slack = pyo.Constraint(m.PH, m.T, rule=slack_fix)
